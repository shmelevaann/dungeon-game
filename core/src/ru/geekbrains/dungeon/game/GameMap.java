...
public class GameMap {
    ...
    public static final int DOUBLE_COST_PERCENTAGE = 5;
    ...
    public GameMap() {
        ...
        int doubleCostCellsCount = (int) ((CELLS_X * CELLS_Y * DOUBLE_COST_PERCENTAGE) / 100.0f);
        for (int i = 0; i < doubleCostCellsCount; i++) {
            this.data[MathUtils.random(0, CELLS_X - 1)][MathUtils.random(0, CELLS_Y - 1)].setCost(2);
        }
    ...
    public void render(SpriteBatch batch) {
        for (int i = 0; i < CELLS_X; i++) {
            for (int j = CELLS_Y - 1; j >= 0; j--) {
                batch.setColor(1, 1.0f / data[i][j].cost, 1, 1);
